    ...
    def draw(
        ...
        if ss > 1:
            for i in range(ss):
                ...
                drawCircleFunc(
                    self.pts[:, 0], color=(255 - color[0], 0, 255 - color[2])
                )
                drawCircleFunc(
                    self.pts[:, -1],
...
def rotation_transform(wh, angles=np.array([0.0, 0.0, 0.0]), zcop=1000.0, dpp=1000.0):
...
class Display:
    ...
    def reset_view(self):
        ...
        self.mouse_center = np.array([0.5, 0.5])
    ...
    def updatePerspectiveMatrix(self):
        ...
        self.dx = self.cx * w * zf - self.width / 2.0
        self.dy = self.cy * h * zf - self.height / 2.0

        R = np.eye(3)
        R = np.matmul(
            R, np.matrix([[zf, 0, -self.dx], [0, zf, -self.dy], [0, 0, 1]], dtype=float)
        )
    ...
    def setPerspectiveAngle(self, addx=0.0, addy=0.0, addz=0.0):
    ...
    def _setPerspective(self, update=True):
        ...
        self.IdisplayCopy = cv2.warpPerspective(
            self.Iorig,
            self.R,
    ...
    def __pt2xy(self, pt):
        pt = np.squeeze(
            np.array(np.matmul(self.R, np.append(pt * getWH(self.Iorig.shape), 1.0)))
        )
        pt = pt[:2] / pt[2]
    ...
    def mouse_callback(self, event, x, y, flags, param):
        ...
        mc = np.matmul(self.Rinv, np.append(mc, 1.0))
    ...
    while curr_image < len(img_files):
        ...
        while not key in action_keys:
            ...
            if key == key_yaw_increase:
                disp.setPerspectiveAngle(addy=10.0)
            if key == key_yaw_decrease:
                disp.setPerspectiveAngle(addy=-10.0)
            if key == key_pitch_increase:
                disp.setPerspectiveAngle(addx=10.0)
            if key == key_pitch_decrease:
                disp.setPerspectiveAngle(addx=-10.0)
            if key == key_roll_increase:
                disp.setPerspectiveAngle(addz=10.0)
